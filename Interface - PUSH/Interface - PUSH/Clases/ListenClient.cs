...
namespace Interface___PUSH.Clases
{
    class ListenClient
    {
        ...
        public void StartListening()
        {
            try
            {
                if (servidorTcp == null)
                {
                    // 1 linea aumentada y 1 línea modificada para revisar el objeto IPAddress
                    IPAddress direccionIpLocal = IPAddress.Parse(serverIP);
                    this.servidorTcp = new TcpListener(direccionIpLocal, port);
                }
                this.servidorTcp.Start();
                listening = true;
                Socket clienteTcp_Socket = null;
                ...
                while (listening)
                {
                    ...
                    try
                    {
                        ...
                        clienteTcp_Socket = this.servidorTcp.AcceptSocket();
                        clienteTcp_Socket.ReceiveBufferSize = MAXBUFFERSIZE;
                        clienteTcp_Socket.SendBufferSize = MAXBUFFERSIZE;
                        Thread.Sleep(500);
                        if (clienteTcp_Socket.Available <= 0)
                            continue;
                        byte[] bReceive = new byte[MAXBUFFERSIZE];
                        clienteTcp_Socket.Receive(bReceive);

                        Analysis(bReceive, clienteTcp_Socket);
                    }
                    catch (Exception)
                    {
                    }
        ...
        private string GetDeviceInitInfo(DeviceModel device)
        {
            ...
            // Admite parámetros de plantilla biométrica multimodal， type Definición de tipo
        ...
        /// </summary>
        private string TraeLaHoraDelServidor(string DevSN, ref string RepString)
        {
            ...
            {
                RepString = DateTime.Now.ToString();
                return "200 OK";
            }
        }
        /// <summary>
        ...
        private void cdataProcess(byte[] bReceive, Socket remoteSocket)
        {
            ...
            if (sBuffer.Substring(0, 3) == "GET") // iclock option
            {
                ...
                // De Marcelo
                if (sBuffer.IndexOfEx("type=time", 0) > 0)
                {
                    ReplyCode = TraeLaHoraDelServidor(SN, ref strReply);
                    SendDataToDevice(ReplyCode, DateTime.Now.ToString(), ref remoteSocket);
                    remoteSocket.Close();
                    return;
                }
                else
            ...
            if (sBuffer.Substring(0, 4) == "POST")
            {
                ...
                //options Enviar información de configuración
        ...
        {
            string sBuffer = Encoding.GetEncoding("gb2312").GetString(bReceive).TrimEnd('\0');
            string cmdstring = "OK\r\n";
            string SN = GetValueByNameInPushHeader(sBuffer, "SN");
        ...
        // Divida la cadena del dispositivo en varios campos y luego encapsule en un objeto de registros de operación
        ...
        //Encapsular información general de huellas dactilares
        private TmpFPModel CreatFP(string template, string SN, bool isBioData)
        {
            ...
            if (isBioData)
            {
                ...
                // Obtenga el número de versión de la huella digital compatible con este dispositivo de la base de datos
        ...
        // Encapsular la información de la plantilla de cara
        ...
        //Encapsular la información del registro de asistencia
        ...
        private void Base64StringToImage(string txtFileName)
        {
            ...
            {
                MessageBox.Show("Base64StringToImage La conversión falló / nException：" + ex.Message);
            }
        }
        ...
        /// <summary>
        /// Obtener la instancia del dispositivo según los parámetros
        /// </summary>
        ...
        private DeviceModel GetDeviceModelByOptions(string devSN, string strOptions)
        {
            ...
            ////MultiBioDataSupport El atributo está vacío，El dispositivo no es compatible con el protocolo de identificación híbrido.
        ...
        /// <summary>
        /// Conversión de cadenas de parámetros，Convertido a un formato de protocolo de reconocimiento híbrido
        /// </summary>
        ...
        /// </summary>
        /// <param name="BioType">Tipos de</param>
        /// <param name="val">Actualizar valor</param>
        /// <param name="vals">Grupo de valor</param>
        /// <returns></returns>
